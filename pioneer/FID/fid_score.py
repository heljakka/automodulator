...
from pioneer.FID.inception import InceptionV3
...
#
#
# def _compute_statistics_of_path(path, model, batch_size, dims, cuda, reco=True):
...
#         if reco:
#             files = list(path.glob('*_pine.png'))
#         else:
#             files = list(path.glob('*_orig.png'))
...
def get_activations_of_images(images, model, batch_size=50, dims=2048,
                              cuda=False):
    """Calculates the activations of the pool_3 layer for all images.
...
def _calculate_activation_statistics_of_images(files, model, batch_size=50,
...
def calculate_fid_given_images(real, reco, batch_size, cuda, dims):
